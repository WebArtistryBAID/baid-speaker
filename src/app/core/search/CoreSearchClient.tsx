...
import AlwaysSearchNav from '@/app/core/search/AlwaysSearchNav'
...
export default function CoreSearchClient({lectures, uploadServePath, query}: {
    ...
    return <>
        <AlwaysSearchNav/>
        <div className="base-studio-page">
            <If condition={query.length < 1}>
                <div className="lg:hidden h-screen w-screen" onClick={() => router.push('/core')}></div>
            </If>
                                ...
                                <If condition={lecture.status === LectureStatus.completingPostTasks || lecture.status === LectureStatus.completed}>
                                    <p className="text-xs secondary">
                                        <If condition={lecture.liveAudience != null}>
                                            <Trans t={t} i18nKey="home.liveViews"
                                                   count={lecture.liveAudience ?? 0}/> •&nbsp;
                                        </If>
